...
**独自ドメインのHTTPSと、公開HPからの最終送信テストは確認中です。** HTTPではフォームの入力を表示せず、メール相談の案内を表示します。HTTPSでは入力・確認・送信・成功／失敗の表示を利用できます。JavaScript無効時もメール相談先を表示します。フォームの内容は確認画面の最終ボタンでのみ送信し、ブラウザ内への永続保存は行いません。
